...
class _LoginAgriculteurState extends State<LoginAgriculteur> {
  ...
  void initState() {
    super.initState();
    // Initialiser les préférences au début de l'application
    getSharedPreferenceValue();
    // Vérifier si l'utilisateur est déjà connecté lors du démarrage
  ...
  void login() {
    ...
    // Mettez à jour l'état et réagissez en conséquence
    setState(() {
      isUserLoggedIn = true;
    });
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => Accueil()));
